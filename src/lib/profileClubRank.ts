export interface ProfileClubRankMember {
  userId: string;
  name: string;
  elo: number;
...
export interface ProfileClubRankMatch {
  team1User1Id: string;
  team1User2Id: string;
  team2User1Id: string;
  team2User2Id: string;
...
export interface ProfileClubRankWindow {
...
export interface ClubLeaderboardRankMovement {
  currentRank: number;
  previousRank: number | null;
  rankDelta: number | null;
}
...
function rankMembers(
  userId: string,
  membersById: Map<string, ProfileClubRankMember>
) {
...
function rankAllMembers(membersById: Map<string, ProfileClubRankMember>) {
...
function buildEligibleMemberMaps(members: ProfileClubRankMember[]) {
...
function rollbackRankWindow(
  previousMembersById: Map<string, ProfileClubRankMember>,
  matchesSinceWindowStart: ProfileClubRankMatch[],
...
export function buildProfileClubRankWindow(
  userId: string,
  members: ProfileClubRankMember[],
  matchesSinceWindowStart: ProfileClubRankMatch[]
): ProfileClubRankWindow {
...
export function buildClubLeaderboardRankMovements({
  members,
  matchesSinceWindowStart,
  ...
  return new Map(
    [...currentRankByUserId.entries()].map(([userId, currentRank]) => {
      ...
      return [
        ...
        } satisfies ClubLeaderboardRankMovement,
      ];
    })
  );
}
