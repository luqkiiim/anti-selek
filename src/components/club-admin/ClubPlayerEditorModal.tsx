...
interface ClubPlayerEditorModalProps {
  ...
  onGrantStaff: (player: ClubAdminPlayer) => Promise<void>;
  onRevokeStaff: (player: ClubAdminPlayer) => Promise<void>;
  onOpenPasswordReset: (player: ClubAdminPlayer) => void;
  canDemoteAdmins: boolean;
  canOpenEmergencyPasswordReset: boolean;
  onUploadAvatar: (player: ClubAdminPlayer, file: File) => Promise<void>;
  onRemoveAvatar: (player: ClubAdminPlayer) => Promise<void>;
}
            ...
            <div className="space-y-2">
              <label className="block space-y-2 text-sm font-medium text-gray-900">
                ...
                <select
                  ...
                  onChange={async (event) => {
                    await onUpdatePreferences(player, {
                      status: event.target.value as ClubPlayerStatus,
                    });
                  }}
                  disabled={savingPreferences}
                  className="field"
              ...
              {player.isOwner ? (
                ...
                canDemoteAdmins ? (
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() => onDemoteAdmin(player, ClubRole.STAFF)}
                      disabled={savingRole}
                      className="app-button-secondary px-4 py-2"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => onDemoteAdmin(player, ClubRole.MEMBER)}
                      disabled={savingRole}
                      className="app-button-secondary px-4 py-2"
                    >
