...
export function PlayerProfilePage({ clubId, clubName, member, isSelf, identityPins, achievements, milestone, onOpenMember }: {
  ...
  return <article className="player-profile" aria-label={`${member.name}’s club profile`}>
    ...
    {data && <>
      ...
      <ProfileHistory clubId={clubId} userId={member.id} history={data.history} onOpen={setRecap} />
    </>}
...
function RatingJourney({ timeline, onOpenSession }: { timeline: MemberProfileData["timeline"]; onOpenSession: (id: string) => void }) {
  ...
  return <section className="profile-journey"><div className="section-heading"><h2>Rating journey</h2><ChartLineUp size={22} weight="duotone" /></div><div className="profile-chart-tabs" aria-label="Rating history range"><button aria-pressed={!all} onClick={() => { setAll(false); setSelectedId(null); }}>Last 10 sessions</button><button aria-pressed={all} onClick={() => { setAll(true); setSelectedId(null); }}>All time</button></div>
    {points.length ? <><div className="profile-chart-touch" onPointerDown={event => { event.currentTarget.setPointerCapture(event.pointerId); trackPoint(event); }} onPointerMove={trackPoint}><svg viewBox="0 0 320 140" className="profile-chart" role="img" aria-label="Recorded club rating over time">{selected && <line x1={x(points.indexOf(selected))} x2={x(points.indexOf(selected))} y1="12" y2="124" stroke="#bca6dc" strokeDasharray="3 4" />}<path d="M18 118H302" stroke="#e7dcef" />{lines.map((line, i) => <polyline key={i} points={line} fill="none" stroke="#7040cf" strokeWidth="3" strokeLinejoin="round" strokeLinecap="round" />)}{points.map((p,i) => p.rating === null ? null : <g key={p.id} onClick={() => setSelectedId(p.id)}><circle cx={x(i)} cy={y(p.rating)} r="24" fill="transparent" /><circle cx={x(i)} cy={y(p.rating)} r={selected?.id === p.id ? 5 : 3} fill={p.kind === "MANUAL" ? "#ad731c" : p.kind === "ACTIVE" ? "#80718b" : "#7040cf"} /></g>)}</svg></div><div className="profile-chart-selection"><label>Selected point<select aria-label="Rating history point" value={selected?.id ?? ""} onChange={e => setSelectedId(e.target.value)}>{points.map(p => <option key={p.id} value={p.id}>{p.label} · {dateLabel(p.date)}</option>)}</select></label>{selected && <><div><strong>{selected.rating ?? "—"}</strong><span>{selected.kind === "MANUAL" ? "Manual adjustment" : selected.kind === "ACTIVE" ? "Session in progress" : selected.kind === "GAP" ? "Historical rating unavailable" : selected.reconstructed ? "Reconstructed rating" : "Club rating"}</span></div><small>{dateLabel(selected.date)}{selected.delta !== null ? ` · ${signed(selected.delta)} rating` : ""}</small>{selected.sessionId && (selected.kind === "SESSION" || selected.kind === "GAP") && <button className="text-button" onClick={() => onOpenSession(selected.sessionId!)}>View session<CaretRight size={15} /></button>}</>}</div>{selected?.reconstructed && <p className="profile-footnote">Reconstructed from saved game changes and the next recorded rating.</p>}{points.some(p => p.kind === "GAP") && <p className="profile-footnote">Gaps indicate ratings that were not recorded.</p>}</> : <p className="profile-footnote">The journey appears once a rating change is recorded.</p>}
  </section>;
}
