...
function RivalryCard({ rivalry, rank }: { rivalry: ClubPulseRivalry; rank: number }) {
  ...
  return <li className="rivalry-card">
    <div className="rivalry-caption"><span>#{rank}</span><span>{rivalry.matches} games against each other</span></div>
    ...
    <div className="rivalry-record" aria-hidden="true"><span style={{ flex: rivalry.playerOneWins }} /><span style={{ flex: rivalry.playerTwoWins }} /></div>
  </li>;
}
...
export function TopRivalries({ rivalries }: { rivalries: ClubPulseRivalry[] }) {
  ...
  return <section className="rivalries-section" aria-label="Top rivalries">
    ...
    <Sheet open={open} title="Top rivalries" onClose={() => setOpen(false)}>
      <ol className="rivalry-list">{rivalries.map((rivalry, i) => <RivalryCard key={rivalry.players.map(p => p.id).join(":")} rivalry={rivalry} rank={i + 1} />)}</ol>
