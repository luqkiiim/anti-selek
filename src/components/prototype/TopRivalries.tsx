...
export function TopRivalries({ rivalries }: { rivalries: ClubPulseRivalry[] }) {
  ...
  return <section className="rivalries-section" aria-label="Top rivalries">
    ...
    {rivalries.length ? <ol className="rivalry-list rivalry-list-joined chemistry-card">{rivalries.slice(0, 3).map((rivalry, i) => <RivalryCard key={rivalry.players.map(p => p.id).join(":")} rivalry={rivalry} rank={i + 1} />)}</ol>
