...
export function MobileBottomTabs<T extends string>({
  ...
  return (
    <nav
      ...
      className={`fixed inset-x-0 bottom-0 z-40 rounded-t-2xl border-t border-gray-200 bg-white/95 px-3 pb-[calc(env(safe-area-inset-bottom)+0.65rem)] pt-2 shadow-[0_-12px_28px_rgba(23,32,31,0.12)] backdrop-blur-md ${visibilityClassName}`}
    >
      <div className="mx-auto flex max-w-md items-center justify-around gap-1 px-2 py-2">
        {items.map((item) => {
