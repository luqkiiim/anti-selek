          ...
          >
            {canReshuffleWithoutPlayer ? (
              <button
                ...
                className={`flex min-w-0 w-full items-center gap-1.5 whitespace-normal break-words text-[0.95rem] font-semibold leading-tight text-gray-900 transition hover:text-blue-700 disabled:cursor-not-allowed disabled:opacity-50 sm:gap-2 sm:text-base md:text-[1.35rem] xl:text-base ${textAlignClass} ${rowDirectionClass}`}
              >
                <Avatar
                  name={player.name}
...
function ScoreSlot({
  ...
  if (canEdit) {
    return (
      <input
        ...
        onBlur={onScoreBlur}
        className="h-12 w-12 rounded-xl border border-blue-200 bg-white text-center text-[1.65rem] font-semibold tabular-nums text-gray-900 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 sm:h-16 sm:w-16 sm:text-3xl xl:h-14 xl:w-14 xl:text-2xl"
        placeholder="0"
      />
    );
  }
  ...
  return (
    <div
      className={`flex h-12 w-12 items-center justify-center rounded-xl border bg-white text-[1.65rem] font-semibold tabular-nums sm:h-16 sm:w-16 sm:text-3xl xl:h-14 xl:w-14 xl:text-2xl ${
        displayScore !== null
...
export function LiveMatchCard({
  ...
  const matchLineup = (
    <div className="grid grid-cols-[minmax(0,1fr)_3rem_3rem_minmax(0,1fr)] items-center gap-1.5 sm:grid-cols-[minmax(0,1fr)_4rem_4rem_minmax(0,1fr)] sm:gap-3 md:gap-4 xl:grid-cols-[minmax(0,1fr)_3.5rem_3.5rem_minmax(0,1fr)] xl:gap-3">
      <TeamNames
        matchId={match.id}
        players={[match.team1User1, match.team1User2]}
  ...
  const matchReasonButton = canShowMatchReason ? (
    <button
      type="button"
      onClick={() => setMatchReasonOpen(true)}
      className="flex min-h-12 min-w-12 items-center justify-center rounded-xl border border-blue-200 bg-white/95 text-blue-700 shadow-sm transition hover:border-blue-300 hover:bg-white hover:text-blue-800 active:scale-95"
      aria-label="Show match reasoning"
      title="Match reasoning"
      data-live-match-reason-button="footer-leading"
    >
    ...
    <div className="space-y-3">
      ...
      {canEdit ? (
        <ScoreEntryControls
          ...
          submitLeadingAction={!isConfirmingSubmission ? matchReasonButton : null}
